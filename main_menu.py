...
class MainMenuClass:
    ...
    def select_difficulty(self,game_tracking_var_object):
        ...
        while (not((string_input.casefold()) in game_tracking_var_object.valid_difficulty_inputs)):
            string_input = input('Invalid input. Enter either \'Easy\', \'Medium\', or \'Hard\':')
        if ((string_input.casefold()) == game_tracking_var_object.valid_difficulty_inputs[0]):
            return game_tracking_var_object.translate_difficulty_to_recursion_levels['Easy']
        elif ((string_input.casefold()) == game_tracking_var_object.valid_difficulty_inputs[1]):
            return game_tracking_var_object.translate_difficulty_to_recursion_levels['Medium']
        else:
            return game_tracking_var_object.translate_difficulty_to_recursion_levels['Hard']

    def select_player_option(self,game_tracking_var_object,file_and_database,single_player_object,\
                             multiplayer_game,ai_object,checker_object,\
                             input_method_object,print_method_object):
        """
            This function runs the grand scheme of the single_player_object, taking care of which
            mode is being played at, the difficulty, the adjsuting of the database, and
            reading it to the file.
        """
        difficulty = None
        ...
        while (not((string_input.casefold()) in game_tracking_var_object.single_or_multiplayer)):
            string_input = input('Invalid input. Enter either \'s\' for single player, or \'m\' for multiplayer:')
        if ((string_input.casefold()) == game_tracking_var_object.single_or_multiplayer[0]):
            if (nones == 0):
                string_input = input('There are 2 people signed in. Select 1 of \'{First_name}\' and\
 \'{Second_name}\' to play as:'.format(First_name = file_and_database.instance_players_signed_in[0],Second_name = file_and_database.instance_players_signed_in[1]))
 ...
 \'{Second_name}\' to play as:'.format(First_name = file_and_database.instance_players_signed_in[0],Second_name = file_and_database.instance_players_signed_in[1]))
                ...
                result = single_player_object.run_single_player(string_input,difficulty,ai_object,\
                                                                checker_object,input_method_object,\
                ...
                file_and_database.instance_database = file_and_database.rank_database()
                ...
                if (file_and_database.instance_players_signed_in[0] == None):
                    difficulty = self.select_difficulty(game_tracking_var_object)
                    result = single_player_object.run_single_player(file_and_database.instance_players_signed_in[1],\
                    ...
                    difficulty = self.select_difficulty(game_tracking_var_object)
                    result = single_player_object.run_single_player(file_and_database.instance_players_signed_in[0],\
                                                    difficulty,ai_object,checker_object,input_method_object,\
                                                    print_method_object,game_tracking_var_object)
                    file_and_database.result_score_single_player(result[0],result[1],\
                                                                 result[2],result[3],\
            ...
            if (nones == 1):
                ...
                if ((len(file_and_database.instance_database)) == 1):
                    print('Not enough names in the database. Noboody will be signed in during the game.')
                    ...
                    (First_name = file_and_database.instance_players_signed_in[0],Second_name = file_and_database.instance_players_signed_in[1]))
                    print('These are the two names signed in. The second name signed in is the second player.')
                    result = multiplayer_game.run_game(file_and_database.instance_players_signed_in[i],name,\
                                                       checker_object,game_tracking_var_object,print_method_object,\
                                                       input_method_object)
                ...
                result = multiplayer_game.run_game('','',checker_object,game_tracking_var_object,\
                                                   print_method_object,input_method_object)
                checker_object.reset_the_board(game_tracking_var_object)
                print_method_object.clear_the_board(game_tracking_var_object)
...
order. When you play multiplayer and win, it\'s +2, when you lose it\'s -2, and when you tie yo get +1.\n')
    ...
    def leaderboards(self,file_and_database,game_tracking_var_object,single_player_object,multiplayer_game,\
                     ai_object,checker_object,input_method_object,print_method_object):
        """
...
to clear all names, \'clear name\' to clear a name, \'b\' to go back to main menu, amd \'p\' to print the leaderboards.\n')
        if ((string_input.casefold()) == 'r'):
            self.print_rules()
            self.leaderboards(file_and_database,game_tracking_var_object,single_player_object,multiplayer_game,\
                              ai_object,checker_object,input_method_object,print_method_object)
            ...
            self.main_menu(file_and_database,game_tracking_var_object,single_player_object,multiplayer_game,\
                           ai_object,checker_object,input_method_object,print_method_object)
    ...
    def main_menu(self,file_and_database,game_tracking_var_object,single_player_object,multiplayer_game,\
                  ai_object,checker_object,input_method_object,print_method_object):
        """
 ...
 username,\n\'g\' to play the game and \'o\' to sign out.\n')
    ...
    \n\'g\' to play the game and \'o\' to sign out.\n')
        if ((string_input.casefold()) == 'l'):
            ...
            Main.select_player_option(game_tracking_var_object,file_and_database,single_player_object,\
                                      multiplayer_game,ai_object,checker_object,\
                                      input_method_object,print_method_object)
            self.main_menu(file_and_database,game_tracking_var_object,single_player_object,multiplayer_game,\
                           ai_object,checker_object,input_method_object,print_method_object)
...
Main.main_menu(file_object,game_tracking_var_object,single_player_object,multi,AI,checker_obj,input_obj,print_obj)
