...
class Checker:
    ...
    def no_one_wins(self,print_fn_object,variable_object):
        ...
        """
        horizontal = self.three_in_a_row(variable_object.current_status_of_board_pieces)
        vertical = self.three_in_a_column(variable_object.current_status_of_board_pieces)
        diagonal = self.diagonals(variable_object.current_status_of_board_pieces)
    ...
    def reset_the_board(self, variable_object):
        ...
        for i in (range(len(variable_object.array_of_available_spots))):
            for j in (range(len(variable_object.array_of_available_spots[i]))):
                ...
                variable_object.current_status_of_board_pieces[i][j] = 0
